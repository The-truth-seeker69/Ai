def collab():
    ...
    # =========================
    @st.cache_resource
    def train_model(ratings_df, seed=42):
        """Trains SVD++ with reproducible hyperparameter tuning."""
        ...
        progress_bar = st.progress(0, text="Tuning model hyperparameters...")
        ...
        for i, (n_factors, n_epochs, lr_all, reg_all) in enumerate(itertools.product(*param_grid.values())):
            ...
            algo = SVDpp(
                ...
                reg_all=reg_all,
                verbose=False,
                random_state=seed  # make SVDpp deterministic
            )

            try:
                results = cross_validate(algo, data.build_full_trainset(), measures=['rmse'], cv=kf, verbose=False)
                ...
                status_text.warning(f"Failed with parameters {best_params}: {e}")

        progress_bar.empty()
        status_text.empty()

        # Train final model with best hyperparameters
        final_algo = SVDpp(**best_params, random_state=seed)
    ...
    # Optional display for final evaluation
